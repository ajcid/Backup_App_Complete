...
def copy_files_for_line(linha, maquina):
    ...
    while not stop_copy_flags.get(thread_key, False):
        try:
            ...
            use_test_mode = linha_cfg.get('use_test_mode', False)
            
            src_path = m_cfg.get('src_test' if use_test_mode else 'src_prod', '')
            dst_path = m_cfg.get('dst_test' if use_test_mode else 'dst_prod', '')
            ...
            if src_path_norm != last_src or dst_path != last_dst:
                mode_str = "TESTE" if use_test_mode else "PROD"
                logging.info(f"[{linha}/{maquina}] Caminhos Ativos ({mode_str}): ORIGEM='{src_path_norm}' | DESTINO='{dst_path}'")
